...
**File:** `../source-code/src/sql/customer_orders_pipeline.sql`
...
### Description
Customer Orders Pipeline - SQL Implementation This SQL script implements the customer orders processing and validation logic Outputs: result_table (valid records) and holding_table (invalid records) Only include orders with status 'COMPLETED' or 'PENDING' Exclude 'CANCELLED' orders from all calculations Only include transactions with status 'SUCCESS' Failed or pending transactions are excluded from payment calculations Step 1: Create filtered orders view Valid orders are those with COMPLETED or PENDING status
...
### Description
Step 2: Create filtered transactions view Only successful transactions count towards payment totals
...
### Description
Step 3: Join orders with transaction summaries Each order is enriched with its total paid amount and transaction count
...
### Description
Step 4: Calculate monthly aggregations per account Aggregate all orders by account and month to calculate: - Total order amount - Number of orders - Total products ordered
...
### Description
Step 5: Enrich with account data Join monthly totals with account information

### Business Rules
- ACCOUNT_ENRICHMENT

### Tables Referenced
- `monthly_account_totals`
- `monthly`
- `accounts`
...
### Description
Step 6: Apply validation rules Records are validated against multiple rules: 1. MISSING_ACCOUNT_ID: account_id must not be null 2. MISSING_CUSTOMER_NAME: customer_name must not be null 3. NEGATIVE_AMOUNT: monthly_total must be >= 0 4. INVALID_ORDER_COUNT: order_count must be > 0 5. MISSING_ORDER_LIMIT: order_limit must not be null

### Business Rules
- VALIDATION_RULES

### Tables Referenced
- `enriched_monthly_totals`
...
### Description
============================================================================= RESULT TABLE OUTPUT ============================================================================= result_table contains all records that pass validation rules
...
### Description
============================================================================= HOLDING TABLE OUTPUT ============================================================================= holding_table contains all records that fail validation rules Each record includes the hold_reason and hold_timestamp

### Business Rules
- HOLDING_TABLE_OUTPUT
...
### Description
============================================================================= SUMMARY QUERIES ============================================================================= Result Summary Query Aggregate metrics for valid records
...
### Description
Holding Summary Query Summary of records in holding table by reason
