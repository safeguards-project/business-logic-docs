...
### Description
Customer Orders Pipeline - SQL Implementation This SQL script implements the customer orders processing and RAG calculation logic Only include orders with status 'COMPLETED' or 'PENDING' Exclude 'CANCELLED' orders from all calculations Only include transactions with status 'SUCCESS' Failed or pending transactions are excluded from payment calculations Step 1: Create filtered orders view Valid orders are those with COMPLETED or PENDING status
...
### Description
Step 5: Calculate month-over-month changes Compare current month total against previous month total Calculate percentage change: (current - previous) / previous * 100
...
### Description
Step 6: Determine RAG status RAG thresholds: - RED: percentage_change >= 50% - AMBER: percentage_change >= 30% AND < 50% - GREEN: percentage_change < 30% OR NULL (new customers)
...
### Description
Step 7: Final RAG Analysis Query Returns the complete RAG analysis for the most recent month Includes all risk indicators and customer information
...
### Description
Risk Summary Query Aggregate counts for monitoring and reporting
